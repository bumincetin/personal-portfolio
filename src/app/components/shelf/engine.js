/**
 * Working Volumes shelf engine.
 *
 * Ported from the registered ThreeUI source for `CompleteShelfLandingPage`:
 * public/landing-pages/complete-shelf-v2.html, SHA-256
 * 606f200fed8602c243f40a11c8c364f0e625c57f80e7c97dc76419da207f198e, verified on
 * download. The scene graph, materials, page-turn rig, orbit handling, focus
 * management and static fallback are the authored implementation, unchanged.
 *
 * Four hosting adaptations, and nothing else (see docs/implementation-summary.md):
 *
 *   1. Addon imports resolve through the installed three@0.165.0 package rather
 *      than the source's CDN importmap. Same version, same files.
 *   2. `BOOKS` arrives as an argument so the shelf can carry this site's seven
 *      volumes, localised, instead of the sample catalogue.
 *   3. The two embedded WebP textures are served from public/shelf/ instead of
 *      inline data URIs. The images are byte-identical; keeping ~712 kB of
 *      base64 inside a JavaScript chunk was not worth the parity.
 *   4. The body is wrapped so React can mount and unmount it. The authored
 *      `disposeExperience` is returned as the teardown, replacing the source's
 *      `beforeunload` listener.
 *
 * This file is generated. Do not hand-edit it — change the generator or the
 * volume data instead.
 */
...
/**
 * Mounts the shelf against the markup already in the document.
 *
 * The authored engine finds its elements by id (#experience, #scene,
 * #detail-panel and the rest), so <Shelf /> must have rendered that structure
 * before this runs.
 *
 * @param {object} options
 * @param {Array} options.books        Seven volume records, in display order.
 * @param {string} options.coverAtlasUrl  3584x768 atlas of seven 512x768 covers.
 * @param {string} options.woodTextureUrl Shelf timber map.
 * @returns {() => void} teardown
 */
export function createShelf({ books, coverAtlasUrl, woodTextureUrl }) {
  const BOOKS = books;
  ...
  function updateSelection(index, announce = false) {
    ...
    // The palette line was removed from the markup: "Ultramarine · bone ·
    // copper" names the colours of a cover, which tells a visitor nothing about
    // the problem inside it. The guard keeps the authored call site intact.
